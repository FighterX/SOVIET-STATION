...
/datum/job/consul
	title = "Consul of Sol Government"
	flag = CONSUL
	department_flag = CIVILIAN
	faction = "Station"
	total_positions = 1
	...
	selection_color = "#dddddd"
	access = list(access_solgov, access_maint_tunnels)
	minimal_access = list(access_solgov)


	equip(var/mob/living/carbon/human/H)
		if(!H)	return 0
		H.equip_to_slot_or_del(new /obj/item/clothing/under/rank/consul(H), slot_w_uniform)
		H.equip_to_slot_or_del(new /obj/item/device/pda/lawyer(H), slot_belt)
