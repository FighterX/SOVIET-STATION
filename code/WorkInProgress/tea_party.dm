...
//Растение
/obj/item/seeds/teaseed					//Семен
	//Параметры беру почти с потолк
	name="pack of tea seeds"
	desc="These seeds grow into tea bushes."
	icon_state="seed-tea"				//В файле seeds.dmi создать спрайт пакета семян.
	mypath="/obj/item/seeds/teaseed"
	species="tea"						//В файле hydroponics.dmi необходимо создать 3 стадии роста,стадию сбора и мёртвое растение:
	//tea-grow1
	//tea-grow2
	//tea-grow3
	//tea-harvest
	//tea-dead
	plantname="Tea Bush"				//Чайный куст
	productname="/obj/item/weapon/reagent_containers/food/snacks/grown/tea_leaf"
	lifespan=60							//Довольно долго живёт
	endurance=10						//Нежные,мало "жизней"
	maturation=10						//Долго растут
	production=8						//Более-менее долго созревает
	yield=8								//Две порции чая за раз.Думаю,что в самый раз с таким уходом
	potency=10							//Немного не понял,что это делает,потому оставлю,как есть.
	plant_type=1						//Сорняк,требует воды
	growthstages=3						//Растёт в 3 стад
	...
	attackby(obj/item/W,mob/user)
		if (istype(W,/obj/item/weapon/milk_jug))
			if (reagents.total_volume+3<=volume)
				...
				Mj.update_icon()
				user.visible_message("[user.real_name] add milk to tea","You add milk to tea")
			...
			if (reagents.reagent_list.len>0)
				...
				if (Sp.full==1)
					if (reagents.total_volume+2<=volume)
						...
						Sp.full=0
						Sp.update_icon()
						user.visible_message("[user.real_name] add sugar to tea","You add sugar to tea")
				else
	...
	attackby(obj/item/W,mob/user)
		if (istype(W,/obj/item/weapon/spoon))
			if (s_left>0)
				...
				S.full=1
				s_left-=2
				user.visible_message("[user.real_name] takes sugar","You taked sugar")
			else
...
/obj/machinery/dry_machine												//Сушилк
	name="tea preparing machine"
	desc="An great technology, that can fermentate and dry leaves for 1-2 minutes!"											//Машина для сушки чая.Зачем она нужна?Сушить чай.
...
/obj/machinery/vending/teamat
	...
	icon_vend="teamat-vend"
	products=list(/obj/item/weapon/reagent_containers/food/drinks/cup=15,
					/obj/item/weapon/reagent_containers/food/drinks/teapot=5,
					/obj/item/weapon/sugar_bowl=3,
					/obj/item/weapon/milk_jug=3,
					/obj/item/weapon/reagent_containers/food/snacks/candy=13)		//Ну,не просто же чай хлебать,конфетки положим.
	vend_delay=15
...
/obj/item/clothing/suit/cape
	...
	armor=list(melee=10,bullet=0,laser=0,energy=0,bomb=0,bio=0,rad=0)	//Пусть будет плотным и немного защищает от ударов!
